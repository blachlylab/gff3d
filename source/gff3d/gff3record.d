...
struct GFF3_Record
{
    ...
    /// Column 4: start; 1-based integer start position of the feature
    @property start() const { return (cast(string)this.raw.splitter('\t').drop(3).front).to!int; }

    /// Column 5: end; closed coordinate integer ending nucleotide position of the feature 
    @property end() const { return (cast(string)this.raw.splitter('\t').drop(4).front).to!int; }
    ...
    **/
    ...
    @property int length() const { return this.end - (this.start-1); }
    /// Relative start === 1
    @property int relativeStart() const { return 1; }
    /// Relative start === the feature length
    @property int relativeEnd() const  { return this.length; }

    /// Genomic coordinate at offset into feature, taking strandedness into account
    @property int coordinateAtOffset(int offset) const
    {
    ...
    @property string canonicalRepresentation() const {
        return this.seqid~":"~this.start.to!string~"-"~this.end.to!string;
    }
